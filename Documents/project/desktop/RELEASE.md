...
последен път** да си инсталира 1.0.4 на ръка. След това всичко е автоматично.
...
2. Вдигни номера на версията в `desktop/package.json` → полето `"version"`.
   Правилото е просто: поправка → `1.0.4`, нова функция → `1.1.0`.
...
| Файл | За какво е |
|---|---|
| `Climby-Setup-1.0.4.exe` | инсталаторът — това сваля човек |
| `Climby-Setup-1.0.4.exe.blockmap` | позволява следващото обновяване да тегли само разликата (няколко МБ вместо 100) |
| `latest.yml` | тук приложението гледа коя е последната версия |
| `Climby-1.0.4-portable.exe` | вариант без инсталиране; **не се обновява сам** |
...
Това сглобява и качва всичко само, с етикет `v1.0.4`.
...
2. **Tag:** `v1.0.4` (буквата `v` + номера от `package.json`).
3. Провлачи **и трите** файла: `Climby-Setup-1.0.4.exe`,
   `Climby-Setup-1.0.4.exe.blockmap` и `latest.yml`.
...
- „Има нова версия: 1.0.4." → работи; свалянето тръгва на заден план.
...
> Здрасти! Climby има нова версия (1.0.4). Ако вече имаш 1.0.2 или по-нова,
> **не пипай нищо** — приложението ще си я свали само и ще те пита да
> рестартираш. Тегли само разликата, няколко мегабайта, не цялото приложение.
>
> Ако още си на 1.0.1 (или изобщо нямаш Climby), изтегли
> `Climby-Setup-1.0.4.exe` оттук:
> https://github.com/Marun1105/my-project/releases
> и я пусни. Ще се сложи върху старата, нищо няма да се загуби. Това е
> последният път, в който трябва да инсталираш на ръка.
>
