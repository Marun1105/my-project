...
# Neon и Supabase дават ДВА адреса за една и съща база: пряк и "през басейн"
# (PgBouncer). Басейнът е за среди, които вдигат по един процес на заявка — не е
# нашият случай, а за нас е капан: той връща връзката в общия басейн след всяка
# транзакция, докато ключалката по-долу живее върху КОНКРЕТНАТА връзка. Тоест
# ключалката се взима и веднага се губи, без грешка и без следа — точно тихият
# провал, срещу който е написана.
#
# Не спираме заради това (базата пак работи), но го казваме на глас веднъж, за да
# не се търси после.
def _warn_if_pooled(url: str) -> bool:
    pooled = "-pooler." in url or ":6543/" in url
    if pooled:
        print(
            "[migrations] адресът на базата минава през басейн (PgBouncer). "
            "Ключалката за схемата няма да работи там — сложи ПРЯКАТА връзка в "
            "DATABASE_URL (тази без '-pooler' и без порт 6543).",
            file=sys.stderr,
        )
    return pooled


_warn_if_pooled(str(engine.url))
...
def _drop_password_not_null() -> bool:
    ...
    Postgres only. SQLite cannot relax a column constraint without rebuilding the
    table, and it does not need to: a fresh database takes the rule from the model
