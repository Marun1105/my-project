...
def send_account_exists_email(to: str) -> None:
    ...
    html = _wrap(
        ...
        "",
        "Някой (най-вероятно ти) опита да се регистрира с този адрес. Профил вече "
        "има — влез с паролата си, а ако си я забравил/а, използвай "
        "„Забравена парола“. Ако не си ти, спокойно игнорирай това писмо: "
        "нищо не е променено.",
    )
    send_email(to, "Вече имаш профил в Climby", html)
